...
이 문서는 oh-my-claudecode(이하 OMC)의 프로젝트 구조, 동작 원리, 에이전트·스킬 시스템을 소스코드 기반으로 정리한 **비교 참고 자료**다. OMC는 Claude Code 네이티브 플러그인이지만, Superpowers 기반 전략과 직접 교체하는 관계가 아니다. Hook 기반 인터셉션의 실구현 방식, 외부 모델(Codex/Gemini) tmux 통합 패턴을 참고하는 용도로 활용한다.
...
이 인터셉션 레이어 패턴은 **[Part 5. Oh My Bridge](/posts/oh-my-bridge)** 초기 구현에서 플러그인 범위에 맞게 축소 채용되었다. 초기에는 `PreToolUse`로 Edit/Write를 가로채는 방식을 시도했으나, 이후 Skill 기반 라우팅으로 전환했다. 현재 Hook 레이어는 `PostToolUse` 하나로 좁혀져 Codex MCP 호출(`mcp__plugin_oh-my-bridge_codex__.*`)만 선별한다.
